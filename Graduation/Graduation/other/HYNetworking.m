...
+ (NSString *)HYnetworking_md5:(NSString *)string;
...
static HYResponseType sg_responseType = kHYResponseTypeJSON;
static HYRequestType  sg_requestType  = kHYRequestTypePlainText;
static HYNetworkStatus sg_networkStatus = kHYNetworkStatusReachableViaWiFi;
...
static inline NSString *cachePath() {
  return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/HYNetworkingCaches"];
}
...
+ (void)clearCaches {
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
    ...
    if (error) {
      NSLog(@"HYNetworking clear caches error: %@", error);
    } else {
      NSLog(@"HYNetworking clear caches ok");
    }
  }
}
...
+ (HYURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(HYDownloadProgress)progress
                               success:(HYResponseSuccess)success
                                  fail:(HYResponseFail)fail {
  ...
  if ([self baseUrl] == nil) {
    if ([NSURL URLWithString:url] == nil) {
      HYLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
      return nil;
    }
  } else {
  ...
  if (httpMethod == 1) {
    if (sg_cacheGet) {
      if (sg_shoulObtainLocalWhenUnconnected) {
        if (sg_networkStatus == kHYNetworkStatusNotReachable ||  sg_networkStatus == kHYNetworkStatusUnknown ) {
          id response = [HYNetworking cahceResponseWithURL:absolute
                                                 parameters:params];
          if (response) {
            if (success) {
...
+ (HYURLSessionTask *)uploadFileWithUrl:(NSString *)url
  ...
  if ([NSURL URLWithString:uploadingFile] == nil) {
    HYLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
    return nil;
  }
  ...
  if (uploadURL == nil) {
    HYLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
    return nil;
  }
...
+ (HYURLSessionTask *)uploadWithImage:(UIImage *)image
  ...
  AFHTTPSessionManager *manager = [self manager];
  HYURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (HYURLSessionTask *)downloadWithUrl:(NSString *)url
  ...
  session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
    ...
    if (error == nil) {
      ...
      if ([self isDebug]) {
        HYLog(@"Download success for url %@",
      ...
      if ([self isDebug]) {
        HYLog(@"Download fail for url %@, reason : %@",
                  [self absoluteUrlWithPath:url],
                  [error description]);
      }
    }
  ...
  @synchronized (self) {
    ...
    if (sg_sharedManager == nil || sg_isBaseURLChanged) {
      ...
      switch (sg_requestType) {
        case kHYRequestTypeJSON: {
          manager.requestSerializer = [AFJSONRequestSerializer serializer];
          break;
        }
        case kHYRequestTypePlainText: {
      ...
      switch (sg_responseType) {
        case kHYResponseTypeJSON: {
          manager.responseSerializer = [AFJSONResponseSerializer serializer];
          break;
        }
        case kHYResponseTypeXML: {
          manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
          break;
        }
        case kHYResponseTypeData: {
...
+ (void)detectNetwork {
  ...
  [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
    if (status == AFNetworkReachabilityStatusNotReachable){
      sg_networkStatus = kHYNetworkStatusNotReachable;
    } else if (status == AFNetworkReachabilityStatusUnknown){
      sg_networkStatus = kHYNetworkStatusUnknown;
    } else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
      sg_networkStatus = kHYNetworkStatusReachableViaWWAN;
    } else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
      sg_networkStatus = kHYNetworkStatusReachableViaWiFi;
    }
  }];
}
...
+ (void)successResponse:(id)responseData callback:(HYResponseSuccess)success {
  if (success) {
    success([self tryToParseData:responseData]);
  }
}

+ (NSString *)HY_URLEncode:(NSString *)url {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
  ...
  if (url) {
    ...
    NSString *key = [NSString HYnetworking_md5:absoluteURL];
    NSString *path = [directoryPath stringByAppendingPathComponent:key];
    
    NSData *data = [[NSFileManager defaultManager] contentsAtPath:path];
    if (data) {
      cacheData = data;
      HYLog(@"Read data from cache for url: %@\n", url);
    }
  }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
  if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
    ...
    if (data && error == nil) {
      ...
      if (isOk) {
        HYLog(@"cache file ok for request: %@\n", absoluteURL);
      } else {
        HYLog(@"cache file error for request: %@\n", absoluteURL);
      }
    }
  }
}
...
+ (void)handleCallbackWithError:(NSError *)error fail:(HYResponseFail)fail {
  if ([error code] == NSURLErrorCancelled) {
